...
function game_init()
	...
	ani_timer = 0
	local start_angle = p1_type=="computer" and rnd(360) or 30
	...
	arrow_pause=3
	p1 = {name="Player 1",score=0,piece='white',is_ai=p1_type == "computer" and true or false}
	p2 = {name="Player 2",score=0,piece='black',is_ai=p2_type == "computer" and true or false}
	p_turn = p1
...
function game_draw()
	...
	elseif not stri.inhole then
		if stri.state == "positioning" and not p_turn.is_ai then
			draw_piece(stri, 1)
			if game_timer>arrow_pause then
				draw_movement_arrows()
			end
			if rotation_allowed then
				draw_roatation_indicator()
			end
		else
			draw_piece(stri)
		end
...
function pause_draw()
	...
	-- Draw the themed panel from the main menu
	rect(24, 34, 104, 93, 7) -- Outer border
	rectfill(25, 35, 103, 92, 6) -- Inner background

	-- Title text
	local title = "- paused -"
	text_outline(title, 64 - (#title * 2), 45, 7, 1)

	-- Draw a highlight bar behind the selected option
	local cursor_y = (pause_selection == 1) and 60 or 75
	rectfill(38, cursor_y - 1, 90, cursor_y + 7, 13) -- Highlight color

	-- Menu Options with outline
	local txt1 = "resume"
	text_outline(txt1, 64 - (#txt1 * 2), 61, 7, 1)

	local txt2 = "quit to menu"
	text_outline(txt2, 64 - (#txt2 * 2), 76, 7, 1)
end
...
function game_over_draw()
	-- Use the same background as the main menu for consistency
	cls(1) -- Dark blue background

	-- Determine the winner
	local winner = ""
	if #p_white_pocketed >= 9 then
		winner = p1.name
	else
		winner = p2.name
	end

	-- Draw the themed panel
	rect(24, 34, 104, 103, 7) -- Outer border
	rectfill(25, 35, 103, 102, 6) -- Inner background

	-- Game Over Text
	local txt = "game over"
	text_outline(txt, 64 - (#txt * 2), 50, 7, 1)

	-- Winner Text
	txt = winner .. " wins!"
	text_outline(txt, 64 - (#txt * 2), 65, 10, 1)

	-- Prompt to continue, with a subtle bobbing animation
	local prompt_y = 90 + sin(time() * 0.15) * 2
	txt = "press ❎ or 🅾️"
	text_outline(txt, 64 - (#txt * 2), prompt_y, 7, 1)
end
...
function draw_movement_arrows()
	...
	yo=(flr((game_timer/60)*3)%2==0) and -3 or -2
	print("⬅️",stri.x-18,stri.y+yo,12)
	print("➡️",stri.x+12,stri.y+yo,12)
end

function draw_roatation_indicator()
	local txt = "⬆️/⬇️ to roatate"
	text_outline(txt, 62-(#txt*2),30 +flr((game_timer/60)*2)%2,7,1)
end
...
function ai_opponent_turn()
    if ai_state == "positioning" then
        ...
        if best_shot then
            ai_target_pos = {x = best_shot.striker_x, y = stri.y}
            ai_target_angle = best_shot.angle
            ai_shot_power = best_shot.power
        else
            -- Fallback for when no shot is found
            ai_target_pos = {x = 32 + rnd(64), y = stri.y}
            ai_target_angle = 270 + rnd(20) - 10
            ai_shot_power = 30 + rnd(20)
        end
        -- Setup for animation
        -- Using existing 'ani' fields on the striker to avoid new globals
        stri.start_x = stri.x
        stri.start_y = stri.y
        stri.ani_x = ai_target_pos.x
        stri.ani_y = ai_target_pos.y
        stri.ani_progress = 0
        ai_state = "moving_to_position"
        
	elseif ai_state == "moving_to_position" then
        -- Animate the movement
        stri.ani_progress = min(1, stri.ani_progress + 0.05) -- Control animation speed
        ...
        if stri.ani_progress >= 1 then
            stri.x = stri.ani_x -- Snap to final position
            ai_state = "aiming"
            stri.state = "aiming"
            ai_timer = game_timer + 60 -- Wait before aiming
        end
...

...

